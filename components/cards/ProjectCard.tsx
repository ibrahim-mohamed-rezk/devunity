  ...
  return (
    ...
    >
      ...
      <div className="relative w-full h-full p-6 flex flex-col justify-between rounded-3xl shadow-xl border border-gray-200/50 backdrop-blur-sm transition-all duration-500 bg-white/90 hover:bg-white/95 hover:shadow-2xl hover:border-[#00ce93]/20">
        ...
        <div className="flex flex-col space-y-4">
          ...
          <div className="relative">
            ...
            <div className="absolute inset-0 w-16 h-16 rounded-2xl bg-gradient-to-br from-[#00ce93]/20 to-[#00a075]/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 blur-sm"></div>
          </div>

          {/* Project Info */}
          <div className="space-y-3">
            <h3 className="text-gray-800 text-2xl font-bold font-['Inter'] leading-tight">
              {cardData.title}
            </h3>
            <div className="flex items-center space-x-2">
              <div className="w-2 h-2 bg-[#00ce93] rounded-full animate-pulse"></div>
              <span className="text-gray-600 text-sm font-medium font-['Inter'] uppercase tracking-wide">
                {cardData.category || "E-Commerce"}
              </span>
            </div>
            <div className="w-12 h-0.5 bg-gradient-to-r from-[#00ce93] to-[#00a075] rounded-full"></div>
          </div>
        </div>
        ...
        <div className="relative flex-1 mt-6">
          <div className="w-full h-[280px] rounded-2xl overflow-hidden shadow-lg relative group/image">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-[#00ce93]/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>

            {/* Floating preview button */}
            <div className="absolute top-4 right-4 w-10 h-10 bg-[#00ce93]/80 backdrop-blur-sm rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 hover:bg-[#00ce93]">
              <svg
        ...
        <div className="flex items-center justify-center mt-4">
          <div className="w-8 h-1 bg-gray-200 rounded-full">
            <div className="w-4 h-1 bg-gradient-to-r from-[#00ce93] to-[#00a075] rounded-full transition-all duration-300 group-hover:w-8"></div>
          </div>
        </div>
      </div>
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(135deg, rgba(40, 40, 42, 0.95) 0%, rgba(0, 0, 0, 0.90) 100%)",
        }}
      >
        {/* Close button */}
        <button
          className="absolute top-6 right-6 w-10 h-10 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center transition-all duration-300 hover:bg-[#00ce93]/80 hover:scale-110"
          onClick={(e) => {
            e.stopPropagation();
        ...
        <div className="flex items-center justify-center space-x-4 mt-8">
          {cardData?.app_store && (
            <a
              ...
              className="w-14 h-14 bg-white/20 backdrop-blur-sm rounded-2xl flex items-center justify-center transition-all duration-300 hover:bg-[#00ce93]/80 hover:scale-110 group/btn"
            >
              <svg
                className="w-7 h-7 text-white transition-transform duration-300 group-hover/btn:scale-110"
                viewBox="0 0 28 35"
                fill="currentColor"
              >
      ...
      {isHovered && (
        <div className="absolute inset-0 pointer-events-none">
          {[...Array(3)].map((_, i) => (
            <div
              key={i}
              className="absolute w-1 h-1 bg-[#00ce93]/60 rounded-full animate-float"
              style={{
                left: `${20 + i * 25}%`,
                top: `${30 + i * 15}%`,
